...
def _extract_training_patches(
    ...
    Positive patches are centred near GT polygon centroids with random jitter.
    ...
    for poly in polygons_micron:
        ...
        for _ in range(patches_per_gt):
            # Random offset up to ±half so the polygon isn't always centred
            jitter_y = rng.integers(-half, half + 1)
            jitter_x = rng.integers(-half, half + 1)
...
def _build_dataset_and_loader(
    ...
    class VilliDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            # Convert to tensors: (1, H, W) for both
...
def _bce_dice_loss(pred, target):
    ...
    # BCE on logits (numerically stable)
...
def _train_model(
    ...
    """Train U-Net with mixed precision and cosine annealing.
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for images, masks in loader:
            ...
            with torch.amp.autocast("cuda", enabled=(device.type == "cuda")):
                preds = model(images)
                loss = _bce_dice_loss(preds, masks)
...
def _run_inference(
    ...
    """Run tiled inference on the downscaled image.

    Returns a binary mask at the downscaled resolution.
    """
    import torch
    ...
    print(
        f"  Inference: {len(tiles)} tiles ({TILE_SIZE}×{TILE_SIZE}, "
    ...
    # Stitch into full probability map
    prob_map = stitch_masks(prob_tiles, (H, W), mode="max")
...
# ===================================================================
# Phase B (cont.) — Mask → Polygons in micron coordinates
# ===================================================================
...
def _mask_to_polygons(
    ...
    """Convert a binary mask (downscaled) to shapely Polygons in micron coords.

    Pipeline: label → contours → scale 4× to full pixel → × PIXEL_SIZE_UM.
    """
    labels = measure.label(mask_ds, connectivity=2)
    ...
    print(f"  Retained {len(polygons)} polygons (area ≥ {min_area_um2} µm²)")
    return polygons
...
def _save_prediction_overlay(
    ...
    pred_overlay = np.ma.masked_where(~mask_ds.astype(bool), mask_ds)
...
def segment(data_path: str, output_dir: str) -> list[Polygon]:
    ...
    model, loss_history = _train_model(loader, output_dir, device)
    ...
    # B10-B13: Tiled inference + stitching + threshold
    pred_mask = _run_inference(model, image_ds, device)
    print(
        f"  Predicted mask: {pred_mask.sum():,} foreground pixels "
        f"({100.0 * pred_mask.sum() / pred_mask.size:.2f}%)"
    )

    # B14-B17: Mask → polygons in micron coords
    print("[Phase B] Extracting polygons...")
    polygons = _mask_to_polygons(pred_mask, min_area_um2=MIN_AREA_UM2)
    ...
    _save_prediction_overlay(image_ds, pred_mask, gt_mask_ds, output_dir)
